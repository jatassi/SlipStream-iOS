...
- One contract subtlety: the rich media-detail screen pulls **extended metadata from a second base path** (`/api/v1/metadata/*`, the admin `apiFetch` client), not `/api/v1/requests` — flagged in [Epic 03](../03-discovery/media-detail-screen.md).
